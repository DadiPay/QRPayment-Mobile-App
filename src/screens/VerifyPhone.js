...
const VerifyPhone = ({ navigation }) => {
  ...
  useEffect(() => {
    pinValue.length === 4 && navigation.navigate('Confirm_Transaction_Pin', { pinValue })
  }, [pinValue])
    ...
    <AppLayout>
      <View style={styles.container}>
        <View style={styles.contentContainer}>
          ...
          <Header title='Verify Phone Number' icon={UnlockWhiteSvg} subtitle='Enter the OTP code sent to your phone number' />


          <View style={styles.pinUiInput}>
            <View style={styles.hiddenPinContainer}>
              {totalBoxes.map((item, key) => (
                <HiddenPin key={key} filled={(pinValue.length > key)} />
              ))}
            </View>
